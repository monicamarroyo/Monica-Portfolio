...
const RepeatTypistContainer = ({ data }) => (
  <header className="text-center pb-5">
    <Container>
      <Img
        alt="img-girl"
        className="img-fluid rounded mb-3"
        fixed={data.imgGirl.childImageSharp.fluid}
      />
    </Container>
  </header>
)

const RepeatTypist = props => (
  <StaticQuery
    query={graphql`
      query MyQuery3 {
        imgGirl: file(relativePath: { eq: "girl.svg" }) {
          childImageSharp {
            fluid(width: 200, height: 200) {
    ...
    `}
    render={data => (
      <RepeatTypistContainer data={data} {...props}>
        {" "}
      </RepeatTypistContainer>
    )}
  />
)
